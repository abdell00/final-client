...
const Restollogin = () => {
  ...
  const { connexion,  } = useAuth()
    ...
    const signUp = async (e) => {
        e.preventDefault()
        try {
            const res = await axios.post('http://localhost:4000/auth/restau/login', {
              email,
          password 
            })
            if(res.data.token) {
...
<button className="font-mono hover:text-pink-700 " onClick={() => navigate('/addres')}>
